...
from transformers import AutoModel, PretrainedConfig, PreTrainedModel, AutoConfig, AutoTokenizer
...
class BiEncoder(PreTrainedModel):
    ...
    def __init__(self, config):
        ...
        #config = AutoConfig.from_pretrained('./mrpc')

        if config.q_model_name is not None:
            #self.question_model = AutoModel.from_pretrained('./run_glue/mrpc/')
        ...
        if config.ctx_model_name is not None:
            #self.ctx_model = AutoModel.from_pretrained('./run_glue/mrpc/')
            self.ctx_model = AutoModel.from_pretrained(config.ctx_model_name)
        else:
            self.ctx_model = None

        #model_path = './run_glue/mrpc/model.safetensors'
        #state_dict = torch.load(model_path)
        #self.question_model.load_state_dict(state_dict)
        #self.ctx_model.load_state_dict(state_dict)
